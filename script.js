...
const numberWords = {
  // This object contains strings associated with each acceptable number input.  It is used to convert a typed number into an acceptable input at the prompt.
  eight: 8,
  nine: 9,
  ten: 10,
  eleven: 11,
...
const generatePassword = () => {
  ...
  while (
    ...
    passwordLengthNumber < 8 ||
    passwordLengthNumber > 128
  ) {
    passwordLengthInput = prompt(
      "Please enter your desired password length (between 8 and 128 characters)"
    );
    ...
    if (passwordLengthInput === null) {
      alert(
        'Password generation canceled, please press the "Generate Password" button to start again.'
      );
      return;
    } else if (passwordLengthNumber < 8 || passwordLengthNumber > 128) {
      //  || !passwordLengthNumber) {
      alert(
        "The password length must be a number between 8 and 128.  Please try again with a valid entry."
      );
  ...
  while (
    !bucketLowerCaseInput == true &&
    !bucketUpperCaseInput == true &&
    !bucketSymbolInput == true &&
    !bucketNumberInput == true
  ) {
    bucketLowerCaseInput = confirm(
      'Press OK to confirm that you would like to include lowercase letters.  Pressing "Cancel" will exclude lowercase letters from the password.'
    );
    bucketUpperCaseInput = confirm(
      'Press OK to confirm that you would like to include UPPERCASE letters.  Pressing "Cancel" will exclude UPPERCASE letters from the password.'
    );
    bucketSymbolInput = confirm(
      'Press OK to confirm that you would like to include special characters (!, @, #, *, etc.).  Pressing "Cancel" will exclude special characters from the password.'
    );
    bucketNumberInput = confirm(
      'Press OK to confirm that you would like to include numbers.  Pressing "Cancel" will exclude numbers from the password.'
    );
    ...
    if (
      bucketLowerCaseInput === false &&
      bucketUpperCaseInput === false &&
      bucketSymbolInput === false &&
      bucketNumberInput === false
    ) {
      ...
      if (whileCounter == 1) {
        alert(
          "Password Generation cancelled.  Please press the button again to generate a new password.  At least one type of character is required."
        );
        return;
      }
      alert(
        "At least one character type needs to be confirmed.  Please start over, and confirm at least one character set."
      );
      whileCounter++;
    }
  }
  ...
  while (
    !(containsLower === bucketLowerCaseInput) ||
    !(containsUpper === bucketUpperCaseInput) ||
    !(containsSymbol === bucketSymbolInput) ||
    !(containsNumber === bucketNumberInput)
  ) {
    ...
    console.log(
      `The password is ${passwordBuild.length} characters long.  Based on user input, the password should be ${passwordLengthNumber} characters long.`
    );

    //section to validate that the generated password contains at least one of each requested character type
    if (bucketLowerCaseInput === true) {
      // checks if character type is required
      for (let j = 0; j < passwordBuild.length && containsLower !== true; j++) {
        // for loop to iterate across passwordBuild string characters
        for (let i = 0; i < bucketLowerCase.length; i++) {
          // for loop to iterate across bucket array elements
          if (passwordBuild[j] == bucketLowerCase[i]) {
            // checks that string character [j] matches [i]
    ...
    if (bucketUpperCaseInput === true) {
      // checks if character type is required
      for (let j = 0; j < passwordBuild.length && containsUpper !== true; j++) {
        // for loop to iterate across passwordBuild string characters
        for (let i = 0; i < bucketUpperCase.length; i++) {
          // for loop to iterate across bucket array elements
          if (passwordBuild[j] == bucketUpperCase[i]) {
            // checks that string character [j] matches [i]
    ...
    if (bucketSymbolInput === true) {
      ...
      for (
        let j = 0;
        j < passwordBuild.length && containsSymbol !== true;
        j++
      ) {
        // for loop to iterate across passwordBuild string characters
        for (let i = 0; i < bucketSymbol.length; i++) {
          // for loop to iterate across bucket array elements
          if (passwordBuild[j] == bucketSymbol[i]) {
            // checks that string character [j] matches [i]
    ...
    if (bucketNumberInput === true) {
      ...
      for (
        let j = 0;
        j < passwordBuild.length && containsNumber !== true;
        j++
      ) {
        // for loop to iterate across passwordBuild string characters
        for (let i = 0; i < bucketNumber.length; i++) {
          // for loop to iterate across bucket array elements
          if (passwordBuild[j] == bucketNumber[i]) {
            // checks that string character [j] matches [i]
